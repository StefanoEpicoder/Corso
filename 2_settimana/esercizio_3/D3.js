...
for (let i = 0; i < starWarsCharacters.length; i++) { // utilizzo la proprietà length per contare gli elementi 
  const personaggi = starWarsCharacters[i];
  characters.push(personaggi.name); //inserisco dentro il mio array vuoto con .push, tutti i nomi della mia nuova costante creata uguale a quella che avevo in precedenza
}
...
for(let i = 0;i< starWarsCharacters.length; i++) {
  const personaggifemminili = starWarsCharacters[i];

  if(personaggifemminili.gender === "female") {
    femaleCharacters.push(personaggifemminili)
  }
}
